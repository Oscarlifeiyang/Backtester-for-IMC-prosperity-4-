...
def prepare_state(state: TradingState, data: BacktestData) -> None:
    ...
    # Same-tick public prints (before our orders match); not part of Prosperity's official API but useful locally.
    by_t = data.trades[state.timestamp]
    state.session_tape = {sym: list(by_t.get(sym, [])) for sym in data.products}
...
def create_activity_logs(
    ...
    last_valid_mid: dict[Symbol, float],
) -> None:
    for product in data.products:
        row = data.prices[state.timestamp][product]
        safe_mid = row.mid_price
        if safe_mid > 0:
            last_valid_mid[product] = safe_mid
        elif product in last_valid_mid:
            # Some datasets contain bad zero mids; use last valid mark for PnL charting.
            safe_mid = last_valid_mid[product]

        product_profit_loss = data.profit_loss[product]

        position = state.position.get(product, 0)
        if position != 0:
            product_profit_loss += position * safe_mid
...
def run_backtest(
    ...
    for timestamp in timestamps_iterator:
        ...
        create_activity_logs(state, data, result, last_valid_mid)
